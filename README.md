...
After a run attempts a version, the stable-shell workflow can dispatch up to six additional queue-drain runs automatically. This fills a new shell's version history without waiting an hour per version. The queue stops when there is no eligible work or its budget is exhausted; scheduled discovery resumes later. Failed versions retain their cooldown and do not block other candidates. Explicit rebuild and accepted-only runs do not drain the queue.

The build-time permission verifier uses the published patcher's read-only no-change check, including both ends of the newer upstream permission slot. A narrowly checked adapter updates the stale 0.6.9 verifier without changing the published desktop. Unknown verifier shapes fail for review; core artifact acceptance and compatibility fingerprints remain required.

When a new Portable baseline advances its bundled core, automatic discovery also requalifies up to five recent, previously successful historical cores below that baseline. Historical success only makes a version eligible for fresh qualification; it does not bypass the new shell's compatibility checks. Deprecated versions remain excluded. Backfilling history preserves the newest accepted core as the default update, so adding rollback choices never pushes a downgrade. Each channel processes one version per run, prioritizing newer eligible versions.
