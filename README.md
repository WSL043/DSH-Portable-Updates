...
If a newer upstream release requires additional adaptation, manually dispatch with `accepted_only=true` to requalify the already accepted core against the latest verified shell. It still checks registry integrity and runs the complete platform build and acceptance matrix before publication. Scheduled runs continue discovering new official releases; this option does not mark a failed new version as accepted.
